...
const test_conn = () => {
    return store.getSSHConnection().then(connection => new Promise((resolve, reject) => {
        conn.on('ready', () => {
            resolve('connected!')
        }).connect(connection);
    }))
...
const ssh_cmd = async (cmd) => {
    const connection = await store.getSSHConnection().catch(e=>console.log('Please reconfigure ssh connection.'));
